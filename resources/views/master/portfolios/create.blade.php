...
<div class="card card-etrm" style="max-width:600px;"><div class="card-header">New Portfolio</div><div class="card-body">
...
<div class="row g-3">
    ...
    <div class="col-md-6">
        ...
        <input type="text" name="long_name" class="form-control" value="{{ old('long_name') }}">
    </div>
    <div class="col-md-6">
        ...
        <select name="business_unit_id" class="form-select">
            ...
            <option value="{{ $bu->id }}" {{ old('business_unit_id') == $bu->id ? 'selected' : '' }}>{{ $bu->short_name }} — {{ $bu->long_name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-6">
        ...
        <select name="linked_portfolio_id" class="form-select">
            ...
            <option value="{{ $lp->id }}" {{ old('linked_portfolio_id') == $lp->id ? 'selected' : '' }}>{{ $lp->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-md-6">
        ...
        <select name="status" class="form-select" required>
            ...
            <option value="{{ $s }}" {{ old('status','Authorized') == $s ? 'selected' : '' }}>{{ $s }}</option>
            @endforeach
        </select>
    </div>
    <div class="col-12 d-flex gap-4">
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="is_restricted" value="1" {{ old('is_restricted') ? 'checked' : '' }}>
            <label class="form-check-label">Restricted Portfolio</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="requires_strategy" value="1" {{ old('requires_strategy') ? 'checked' : '' }}>
